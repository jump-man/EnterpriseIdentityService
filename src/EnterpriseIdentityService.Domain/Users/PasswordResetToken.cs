...
public sealed class PasswordResetToken : Entity<PasswordResetTokenId>
{
    private PasswordResetToken(
        PasswordResetTokenId id, UserId userId, string tokenHash,
        DateTimeOffset createdAtUtc, DateTimeOffset expiresAtUtc) : base(id)
    {
        if (string.IsNullOrWhiteSpace(tokenHash))
            throw new ArgumentException("Token hash is required.", nameof(tokenHash));
        if (expiresAtUtc <= createdAtUtc)
            throw new ArgumentOutOfRangeException(nameof(expiresAtUtc), "Expiration must be after creation.");
    ...
    public bool IsUsable(DateTimeOffset nowUtc) => !IsConsumed && !IsRevoked && !IsExpired(nowUtc);

    public void Consume(DateTimeOffset occurredOnUtc)
    {
        if (!IsUsable(occurredOnUtc)) throw new InvalidOperationException("Only a usable token can be consumed.");
        ConsumedAtUtc = occurredOnUtc;
    }

    public void Revoke(DateTimeOffset occurredOnUtc)
    {
        if (IsConsumed || IsRevoked) throw new InvalidOperationException("Only an active token can be revoked.");
